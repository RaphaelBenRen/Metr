...
try {
    ...
    $updateQuery = "UPDATE project_shares SET status = 'accepted', updated_at = CURRENT_TIMESTAMP WHERE id = :share_id";
    $stmt = $db->prepare($updateQuery);
    $stmt->bindParam(':share_id', $shareId);
    $stmt->execute();

    jsonSuccess(['message' => 'Partage accepté avec succès']);
} catch(PDOException $e) {
    error_log("Accept share error: " . $e->getMessage());
    jsonError('Erreur lors de l\'acceptation du partage', 500);
}
