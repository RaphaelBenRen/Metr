...
if (!isset($data['library_id']) || !isset($data['user_id'])) {
    jsonError('library_id et user_id sont requis');
}
...
try {
    ...
    if ($library['user_id'] != $userId) {
        jsonError('Seul le propriétaire de la bibliothèque peut retirer l\'accès', 403);
    }

    // Retirer le partage
    $query = "DELETE FROM library_shares
    ...
    error_log("Library unshare error: " . $e->getMessage());
    jsonError('Erreur lors du retrait de l\'accès', 500);
}
