...
import { Plus, Search, MoreVertical, Upload, Edit, Trash2, Filter, SortAsc, FolderKanban, UserCheck } from 'lucide-react'
...
export function ProjectsPage() {
  ...
  const filteredAndSortedProjects = useMemo(() => {
    let filtered = projects.filter(project => {
      ...
      // Ownership filter - convertir en booléen car le backend peut retourner 0/1
      const isOwner = Boolean(project.is_owner)
      const matchesOwnership =
        ownershipFilter === 'all' ||
        (ownershipFilter === 'owned' && isOwner) ||
        (ownershipFilter === 'shared' && !isOwner)

      return matchesSearch && matchesStatus && matchesPhase && matchesTypology && matchesClient && matchesYear && matchesArchived && matchesOwnership
    })
    ...
    return filtered
  }, [projects, searchTerm, statusFilter, phaseFilter, typologyFilter, clientFilter, yearFilter, ownershipFilter, showArchived, sortBy])
  ...
  const hasActiveFilters = statusFilter !== 'all' || typologyFilter !== 'all' ||
    clientFilter !== 'all' || yearFilter !== 'all' || ownershipFilter !== 'all' || showArchived || searchTerm !== ''
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Select value={ownershipFilter} onValueChange={setOwnershipFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="owned">Mes projets</SelectItem>
            ...
            <Card key={project.id} className="hover:shadow-xl transition-all group relative flex flex-col h-full">
              <CardContent className="p-6 flex flex-col items-center text-center h-full">
                ...
                <div className="mb-4 relative">
                  ...
                  {!Boolean(project.is_owner) && (
                    <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-1 shadow-md" title="Projet partagé">
